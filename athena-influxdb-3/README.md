...
| `influxdb_database` (`InfluxDBDatabase`) | Optional default database; if empty, all accessible databases are exposed. If included, only the supplied database may be accessed. Ideally, permissions should be scoped at the token level. |
